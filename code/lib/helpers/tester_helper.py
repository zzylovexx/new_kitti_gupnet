...
class Tester(object):
    ...
    def test(self):
        ...
        for batch_idx, (inputs, calibs, coord_ranges, targets, info) in enumerate(self.data_loader):
            ...
            for key in targets.keys(): targets[key] = targets[key].to(self.device) #in inference use target 
            # the outputs of centernet
            outputs = self.model(inputs,coord_ranges,calibs,targets,K=50,mode='test', use_2dgt=True)
