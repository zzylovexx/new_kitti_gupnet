...
def compute_projection_2dbox(h, w, l, x, y, z, yaw, camera): #input is h,w,l,x,y,z,yaw and gt h,w,l,x,y,z  
    """
    Return : 3xn in cam2 coordinate
    """
    R = np.array([  [np.cos(yaw), 0, np.sin(yaw)], 
                    [0, 1, 0],
                    [-np.sin(yaw), 0, np.cos(yaw)]]) #(3,3)
    x_corners = [l/2,l/2,-l/2,-l/2,l/2,l/2,-l/2,-l/2]
    y_corners = [0,0,0,0,-h,-h,-h,-h] #這種寫法為車子在底部
    z_corners = [w/2,-w/2,-w/2,w/2,w/2,-w/2,-w/2,w/2] #vstack 為沿著第一個維度疊起來 ()
    corners_3d_cam2 = np.dot(R, np.vstack([x_corners,y_corners,z_corners])) #np.vstack([x_corners,y_corners,z_corners])->(3,8)
    corners_3d_cam2 += np.vstack([x, y, z]) #(3,8) + (3,1) broadcast to (3,8) ->(3,8)
...
class Calibration(object):
    ...
    def rect_to_img(self, pts_rect):
        ...
        pts_rect_depth = pts_2d_hom[:, 2] - self.P2.T[3, 2]  # depth in rect_camera(P0) coord
    ...
    def affine_transform(self,img_size,trans):
        ...
        new_calib_matrix[3,:] = new_calib[3:6]
        new_calib_matrix[-1,-1] = self.P2[-1,-1]
        # self.P2 = new_calib_matrix.T #經過affine transform camera projection matrix 改變 
